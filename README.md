...
## Note: Processed Features vs. Raw I/Q Signals
Please note that the `.jsonl` files in this repository contain **pre-processed spatial features**, NOT raw I/Q signal samples. 
To optimize system performance and make the data **immediately ML-ready**, the raw I/Q files generated by the O-RAN xApp are processed in real-time by our Python data pipeline.
...
- How to use: Plot the timestamp on the X-axis and the 4 db values on the Y-axis. You should look for sudden, sharp dips in the db values from each antenna for the walk-bys. 
- Also plot the amp_db or phase waterfalls (e.g., using matplotlib.pyplot.imshow) to observe how frequency-selective fading occurs during the exact moments of the disturbance.
...
- How to use: Extract the x and y coordinates and plot them on a 2D scatter plot to visualize the physical trajectory.
...
To help researchers easily interpret and utilize the collected datasets, we provide the Visualize_Trajectory.py script. This tool parses the .jsonl files, flattens the nested JSON structures into a pandas.DataFrame, and automatically generates analytical charts.

**Usage**

Run the script in your environment to visualize the data:
```
python3 analyze_jsonl.py
```
(Note: Ensure you have updated the dataset_file variable inside the script to point to your target JSONL file, e.g., dataset_single_ue.jsonl)

**Generated Outputs**

- Fig1_Trajectory.png (Spatial Trajectory):
Plots the actual 2D movement path of the UE relative to the Base Station (RU at 0,0). This is useful for assessing the smoothness and accuracy of the localization model over time.
- Fig2_Signal_Strength.png (Antenna dB over Time):
